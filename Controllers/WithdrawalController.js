...
export const requestWithdrawal = async (req, res) => {
  try {
    ...
    if (method === "bank") {
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "Bank details required: accountNumber, ifscCode, accountHolderName",
        });
      }
    } else if (method === "upi") {
      ...
      if (
        ...
        paymentDetails.cryptoAddress.length < 10
      ) {
        return res.status(400).json({
          success: false,
    ...
    if (user.wallet < amount) {
      return res.status(400).json({
        success: false,
        message: `Insufficient balance. Your balance is ₹${user.wallet}`,
    ...
    const referenceId = `WD_${Date.now()}_${userId.toString().slice(-6)}`;
