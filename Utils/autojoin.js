...
export const joinBinaryAuto = async (user) => {
  try {
    ...
    if (!sponsorId) {
      console.log("No sponsor (parentUnilevel) found - cannot auto join binary");
      return;
    }
    ...
    console.log("Binary Sponsor found:", sponsor.name);
    ...
    if (!finalParent.left) {
      ...
      if (!found) {
        console.log("Binary tree full under sponsor");
        return;
      }
    }
    ...
    console.log(
      `Binary joined: ${user.name} at ${finalParent.name}'s ${finalPosition}`,
    );

    // Distribute binary income after joining
    try {
      await distributeBinaryIncome(user._id);
      console.log("Binary income distributed");
    } catch (incErr) {
      console.log("Binary income distribution error:", incErr.message);
    }
...
export const joinMatrixAuto = async (user) => {
  try {
    ...
    const sponsorId = user.parentUnilevel;
    if (!sponsorId) {
      console.log("No sponsor (parentUnilevel) found - cannot auto join matrix");
      return;
    }
    ...
    console.log("Matrix Sponsor found:", sponsor.name);
    ...
    if (!finalParent.leftMatrix) {
      ...
      while (queue.length && !found) {
        ...
        if (current.leftMatrix)
          queue.push(await User.findById(current.leftMatrix));
        if (current.middleMatrix)
          queue.push(await User.findById(current.middleMatrix));
        if (current.rightMatrix)
          queue.push(await User.findById(current.rightMatrix));
      }

      if (!found) {
        console.log("Matrix tree full under sponsor");
        return;
      }
    }
    ...
    console.log(
      ` Matrix joined: ${user.name} at ${finalParent.name}'s ${finalPosition}`,
    );

    // Distribute matrix income after joining
    try {
      const plan = user.plans?.find((p) => p.name === "Matrix");
      if (plan) {
        await distributeMatrixIncome(user, plan.amount);
        console.log(" Matrix income distributed");
      }
    } catch (incErr) {
      console.log("Matrix income distribution error:", incErr.message);
    }
...
export const joinUnilevelAuto = async (user) => {
  try {
    ...
    const sponsorId = user.parentUnilevel;
    if (!sponsorId) {
      console.log("No sponsor (parentUnilevel) found - cannot auto join unilevel");
      return;
    }

    const sponsor = await User.findById(sponsorId);
    if (!sponsor) {
      console.log("Unilevel sponsor not found");
      return;
    }

    console.log("Unilevel Sponsor found:", sponsor.name);

    const alreadyChild = sponsor.childrenUni?.some(
      (childId) => childId.toString() === user._id.toString()
    );

    if (alreadyChild) {
      console.log("User already in sponsor's childrenUni - skipping duplicate add");
    } else {
    ...
    try {
      ...
      if (plan) {
        ...
        console.log(" Unilevel income distributed");
      }
    } catch (incErr) {
      console.log("Unilevel income distribution error:", incErr.message);
    }
